namespace RetroShadersPro.URP
{
    ...
    public class CRTSettings : VolumeComponent, IPostProcessComponent
    {
        ...
        [Header("Resolution Scaling Settings")]

        [Tooltip("Scale parameters based on the reference screen size? Enable if you want pixelation, scanline, and RGB effects to scale seamlessly with screen size.")]
        public BoolParameter scaleParameters = new BoolParameter(false);

        [Tooltip("Base vertical resolution to use as a reference point for scaling properties.")]
